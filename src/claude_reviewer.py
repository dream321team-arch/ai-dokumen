...
# Tool schema enforcing structured JSON output matching BlockReviewResult
REVIEW_TOOL_SCHEMA = {
    "name": "submit_block_review",
    "description": "Submit structured document compliance review result for a block.",
    "input_schema": {
        "type": "object",
        "properties": {
            "block_id": {
                "type": "string",
                "description": "ID of the block under review",
            },
            "original_text": {
                "type": "string",
                "description": "Original block text",
            },
            "status": {
                "type": "string",
                "enum": ["sesuai", "perlu_revisi", "ejaan_tanda_baca", "tidak_ditemukan_rujukan"],
                "description": "Compliance & grammar review status relative to guidelines and PUEBI/EYD",
            },
            "issue": {
                "type": "string",
                "description": "Explanation of non-compliance or error found",
            },
            "rule_reference": {
                "type": "object",
                "description": "Reference to guideline document. REQUIRED if status is perlu_revisi.",
                "properties": {
                    "document": {
                        "type": "string",
                        "description": "Filename of guideline document",
                    },
                    "page": {
                        "type": "integer",
                        "description": "Page number in guideline document",
                    },
                    "section": {
                        "type": "string",
                        "description": "Section or chapter title if available",
                    },
                },
            ...
            "suggested_revision": {
                "type": "string",
                "description": "Concrete suggested revision text to bring block into compliance",
            },
            "span_errors": {
                "type": "array",
                "description": "Specific word/punctuation/phrase span errors inside the text block",
                "items": {
                    "type": "object",
                    "properties": {
                        "original_snippet": {
                            "type": "string",
                            "description": "Kutipan PERSIS teks yang salah dari blok asli (case-sensitive, termasuk tanda baca)",
                        },
                        "suggested_snippet": {
                            "type": "string",
                            "description": "Perbaikan yang benar untuk menggantikan teks salah",
                        },
                        "error_type": {
                            "type": "string",
                            "enum": ["pedoman", "ejaan", "tanda_baca", "kosa_kata"],
                            "description": "Kategori kesalahan: pedoman (pelanggaran aturan), ejaan (typo/kapital/imbuhan), tanda_baca (titik/koma/spasi/dll), kosa_kata (kata tidak baku KBBI)",
                        },
                        "explanation": {
                            "type": "string",
                            "description": "Penjelasan mengapa salah dan aturan apa yang dilanggar",
                        },
                        "severity": {
                            "type": "string",
                            "enum": ["high", "medium", "low"],
                            "description": "Tingkat keparahan: high (kesalahan fatal), medium (perlu perbaikan), low (saran perbaikan)",
                        },
                    },
                    "required": ["original_snippet", "suggested_snippet", "error_type", "explanation"],
                },
            },
        },
        "required": ["block_id", "original_text", "status"],
    },
}
...
- Panggil tool 'submit_block_review' untuk mengirimkan hasil analisis lengkap.
"""


class ClaudeReviewer:
    """Wrapper for Anthropic SDK to review document blocks using structured output tools."""
    ...
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        ...
        if self.api_key and self.api_key.strip():
            try:
                ...
                logger.warning(f"Gagal menginisialisasi Anthropic client: {e}")
        else:
            logger.warning("ANTHROPIC_API_KEY belum diisi di .env file.")
    ...
    def review_block(
        ...
        """
        Reviews a document block against retrieved reference chunks using Claude tool output.

        Args:
            block: The Block object to review
            retrieved_chunks: List of reference Chunk objects
            max_retries: Retry attempts on API/validation failure
        ...
        user_prompt = (
            ...
            f"Panggil tool 'submit_block_review' untuk mengirimkan hasil."
        )
        ...
        if self.client is None:
            return BlockReviewResult(
                ...
                issue="ANTHROPIC_API_KEY belum dikonfigurasi di file .env.",
            )
        ...
        for attempt in range(max_retries + 1):
            try:
                response = self.client.messages.create(
                    ...
                    tool_choice={"type": "tool", "name": "submit_block_review"},
                ...
                for content in response.content:
                    if (
                        ...
                        and content.name == "submit_block_review"
                    ):
                        tool_input = content.input
                        # Guarantee block_id and original_text match input
                        tool_input["block_id"] = block.block_id
                        tool_input["original_text"] = block.text

                        # Validate and fix span_errors
                        raw_span_errors = tool_input.get("span_errors", [])
                        if isinstance(raw_span_errors, list):
                            tool_input["span_errors"] = self._validate_and_fix_span_errors(
                                raw_span_errors, block.text
                            )
                        else:
                            tool_input["span_errors"] = []

                        result = BlockReviewResult(**tool_input)

                        # Check rule_reference requirement for perlu_revisi
                        if result.status == "perlu_revisi" and not result.rule_reference:
                            if retrieved_chunks:
                                # Fallback rule reference from top retrieved chunk
        ...
        # Fallback response in case of total API failure
        return BlockReviewResult(
            block_id=block.block_id,
            original_text=block.text,
            status="tidak_ditemukan_rujukan",
            issue=f"Gagal memproses analisis AI: {str(last_exception)}",
        )
